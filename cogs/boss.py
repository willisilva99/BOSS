...
class BossCog(commands.Cog):
        ...
        async with self.bot.pool.acquire() as connection:
            for member in channel.members:
                if member.id != self.bot.user.id:
                    ...
                    # Aplica infecção ao jogador
                    await connection.execute("UPDATE players SET infected = TRUE WHERE user_id = $1", member.id)
        if infected_players:
            embed = discord.Embed(
    ...
    # 🔥 **Novo: Comandos Relacionados às Snipers**

    # 🔥 **Comando: claim_sniper**
    @boss.command(name="claim_sniper")
